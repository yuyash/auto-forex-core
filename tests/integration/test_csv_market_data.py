...
def test_csv_source_loads_tick_and_candle_files_together(tmp_path: Path) -> None:
...
def test_csv_source_filters_market_data_by_instrument_and_time(tmp_path: Path) -> None:
    tick_path = tmp_path / "ticks.csv"
    tick_path.write_text(
        "\n".join(
            [
